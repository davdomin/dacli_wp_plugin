...
function dacli_settings_page() {
    ...
    if ( isset( $_POST['dacli_clear_cache'] ) && check_admin_referer( 'dacli_cache_action' ) ) {
        ...
        echo '<div class="updated"><p>Caché eliminada.</p></div>';
    }
    ...
    <div class="wrap">
        ...
        <hr>
        <p>Usa el shortcode <code>[dacli_lista]</code>. Atributos opcionales: <code>columns="2" show_phone="yes"</code>, etc.</p>
    </div>
    <?php
}

// 3. OBTENER DATOS DE LA API (con caché)
function dacli_get_labs_data() {
    ...
    if ( false === $labs ) {
        ...
        if ( wp_remote_retrieve_response_code( $response ) !== 200 ) {
            error_log( '[DACLI] La API respondió con código ' . wp_remote_retrieve_response_code( $response ) );
            return [];
        }
...
// 4. RENDERIZADO FRONTEND (shortcode)
function dacli_render_labs_html( $atts = [] ) {
    ...
    if ( empty( $labs ) ) {
        return '<div class="dacli-error" style="text-align:center; padding:20px;">⚠️ No se pudieron cargar los laboratorios. Verifica la configuración.</div>';
    }
    ...
    $output = '<style>
        ...
        .dacli-card { border: 1px solid #e1e8ed; padding: 25px; border-radius: 12px; background: #fff; box-shadow: 0 5px 15px rgba(0,0,0,0.05); transition: all 0.3s ease; }
        .dacli-card:hover { transform: translateY(-7px); border-color: #0d6efd; }
        .dacli-logo { max-height: 70px; width: auto; margin-bottom: 20px; }
        .dacli-name { font-size: 1.3rem; font-weight: bold; margin: 0 0 10px 0; }
        ...
        .dacli-btn { display: inline-block; background-color: #0d6efd; color: #fff !important; text-decoration: none; padding: 10px 20px; border-radius: 6px; font-weight: bold; margin-top: 15px; transition: background 0.2s; }
    ...
    foreach ( $labs as $lab ) {
        $nombre   = esc_html( $lab['nombre_lab'] ?? 'Laboratorio' );
        $nombre_corto = esc_attr( $lab['nombre_lab_corto'] ?? '' );
        $rif      = esc_html( $lab['rif'] ?? '' );
        $direccion= esc_html( $lab['direccion_lab'] ?? '' );
        $testimonio = esc_html( $lab['testimonio'] ?? '' );
        $telefono = esc_html( $lab['telefono_lab'] ?? '' );
        $web      = esc_url( $lab['web_laboratorio'] ?? '' );
        $logo_src = esc_url( $lab['logo'] ?? '' );
        ...
        // Botón solo para web (sin botón llamar)
        if ( $show_web && ! empty( $web ) && $web !== 'http:' ) {
            $output .= '<a href="' . $web . '" class="dacli-btn" target="_blank" rel="noopener noreferrer">🌐 Visitar sitio web</a>';
        }
...
add_shortcode( 'dacli_lista', 'dacli_render_labs_html' );<?php
/**
 * Plugin Name: DACLI WP Plugin
 * Description: Gestión profesional de laboratorios vía API para dAcli Sistemas.
 * Version: 1.3.0
...
function dacli_render_labs_html( $atts = [] ) {
    ...
    foreach ( $labs as $lab ) {
        ...
        if ( ! empty( $buttons ) ) {
            $output .= '<div class="dacli-buttons">' . $buttons . '</div>';
        }
